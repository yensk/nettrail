...
class Filter:
    ...
    def is_host_filtered(self, host):
        if len(self.whitelist_hosts) > 0 and not(host["hostname"] in self.whitelist_hosts or host["host_ip"] in self.whitelist_hosts):
            return True
        if len(self.blacklist_hosts) > 0 and (host["hostname"] in self.blacklist_hosts or host["host_ip"] in self.blacklist_hosts):
            return True
        return False

    def is_port_filtered(self, port):
        if len(self.blacklist_ports)>0 and port["portid"] in self.blacklist_ports:
            return True
        if len(self.whitelist_ports)>0 and not port["portid"] in self.whitelist_ports:
            return True
        return False

def parse_nmap_xml(file_path,filter=Filter()):
    res = {}
    ...
    try:
        res["host_ip"] = tree.find("*/address").attrib["addr"]
    except:
        res["host_ip"] = "unknown"

    try:
        res["hostname"] = tree.find("*/hostnames/hostname").attrib["name"]
    except:
    ...
    for port in ports:
        ...
        p["portid"]  = int(port.attrib['portid'])
        p["state"] = port.find("state").attrib['state']

        try:
            p["service"] = port.find("service").attrib
        except:
            p["service"] = {}

        if not filter.is_port_filtered(p):
...
def join_host_info(host1, host2):
    ...
    if not "host_ip" in host1 or host1["host_ip"] == "unknown":
        host1["host_ip"] = host2["host_ip"]

    if not "ports" in host1:
        host1["ports"] = {}
    for port in host2["ports"]:
        host1["ports"][port] = host2["ports"][port]

    return host1

def generate_flat_service_list(hosts, directory):
    services = collections.defaultdict(list)

    for host in hosts.values():
        for port in host["ports"].values():
            port_desc = f"{port['portid']:>5}"
            ...
            services[port_desc].append(host)
    
    with open(os.path.join(directory,"services.flatlist"), "w") as f:
...
def find_eq_classes(hosts, directory):
    # Sort in equivalence classes
    eq_classes = collections.defaultdict(list)
    for host in hosts.values():
        eq_class = get_ports_fingerprint(host["ports"])
        eq_classes[eq_class].append(host)
    ...
    logger.info("- SUMMARY:")
    logger.info("----------")
    with open(os.path.join(directory,"eq_classes.freq"), "w") as f:
        for eq_class in sorted(eq_classes, key=lambda c: len(eq_classes[c])):
            out = f"{len(eq_classes[eq_class]):>9} : {eq_class}"
            f.write(out+"\n")
            logger.info(out)
    ...
    logger.info("- MAPPING:")
    logger.info("----------")
    with open(os.path.join(directory,"eq_classes.members"), "w") as f:
        for eq_class in sorted(eq_classes, key=lambda c: len(eq_classes[c])):
            out = f"{eq_class}: "+', '.join([helper.get_host_id(x) for x in eq_classes[eq_class]])
            f.write(out+"\n")
            logger.info(out)

# Calculates a fingerprint string for a given port combination
def get_ports_fingerprint(ports):
    fingerprint_input = []
    for port in sorted(ports.keys()):
        if int(port) < 50000 and int(port) > 49000:
            continue
        fingerprint_input.append(f"{ports[port]['protocol']}-{port}")
    return "-".join(fingerprint_input)
...
def nmap_xml_is_finished_run(filename, ports):
    ...
    with open(filename, "r") as f:
        ...
        return True, ports_to_scan


def find_hosts(search_str, ports, folder):
    hosts = {}
    if ports != None and len(ports) > 0:
        hosts = parse_all_hosts(folder)
            ...
            with open(os.path.join(root,file), "r") as f:
                for line in f.readlines():
                    if search_str.lower() in line.lower():
                        ...
                        if ports != None and len(ports) > 0:
                            for host in hosts.values():
                                if host["hostname"].upper() != hostname.upper():
                                    continue

                                if len(host["ports"].keys() & ports) != 0:
